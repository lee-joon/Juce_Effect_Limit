...
//==============================================================================
LimitPluginAudioProcessorEditor::LimitPluginAudioProcessorEditor (LimitPluginAudioProcessor& p)
    : AudioProcessorEditor (&p), audioProcessor (p)
{
	...
	{
		audioProcessor.limiterThresh = std::pow(10, (threshold.getValue() / 20));	//db to linear
	};

	addAndMakeVisible(at);
	//at.setRange(0.f, 10.f, 0.001);
	at.setRange(0.f, 1.f, 0.001);
	at.onValueChange = [this]
	{
		...
		audioProcessor.attackTime = at.getValue();
	};

	addAndMakeVisible(rt);
	//rt.setRange(0.f, 10.f, 0.001);
	rt.setRange(0.f, 1.f, 0.001);
	rt.onValueChange = [this]
	{
		...
		audioProcessor.releaseTime = rt.getValue();
	};
}

LimitPluginAudioProcessorEditor::~LimitPluginAudioProcessorEditor()
{
}

//==============================================================================
void LimitPluginAudioProcessorEditor::paint (juce::Graphics& g)
{
...
void LimitPluginAudioProcessorEditor::resized()
{
